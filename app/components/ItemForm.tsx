...
export default function ItemForm({ state, item, users }: Props) {
  ...
  const { id, name, amount, imageUrl, tags, claimId, description, userId } =
    item || {};
  ...
  return (
    <fetcher.Form
      ...
      method="post"
    >
      {state === 'edit' && <input type="hidden" name="id" value={id} />}

      <div className="flex flex-row gap-5">
        <Label
          className="flex-1"
          caption="Wat is het?"
          subCaption="Oftewel: hoe heet het item?"
        >
        ...
        <Label
          className="flex-1"
          caption="Wat is het bedrag?"
          subCaption="Vanaf-prijs, gebruik een punt i.p.v. comma"
        >
          <NumberInput
            name="amount"
            icon="euro-sign"
      ...
      <div className="flex h-32 flex-row gap-5">
        <Label
          className="flex-1"
          caption="Afbeeldings URL"
          subCaption="Copy & paste van een website naar keuze"
        >
        ...
        <Label
          className="flex-1"
          caption="Tags"
          subCaption="Onder welke categoriëen valt dit?"
        >
